...
class WmsLayerUrbaTreeNode(FavoritesTreeNode):
    ...
    def run_add_to_map_action(self):
        """
        Add the Urba WMS layer with the specified style to the map
        """
        qgis_layer_details = self.get_qgis_layer_details()
        PluginGlobals.instance().iface.addRasterLayer(
            qgis_layer_details["uri"],
...
class WmsLayerPlanTreeNode(FavoritesTreeNode):
    ...
    def get_qgis_layer_details(self):
        ...
        qgis_layer_uri_details = {
            ...
            "provider": "wms",
            "title": self.title,
            "uri": u"crs={}&dpiMode=7&format={}&layers={}&styles={}&tileMatrixSet=LAMB93&url={}&http-header:referer=".format(
                self.layer_srs,
                self.layer_format,
                self.layer_name,
                self.layer_style_name,
...
class WmsLayerOrthoTreeNode(FavoritesTreeNode):
    ...
    def get_qgis_layer_details(self):
        ...
        qgis_layer_uri_details = {
            ...
            "provider": "wms",
            "title": self.title,
            "uri": u"crs={}&fdpiMode=7&format={}&layers={}&styles={}&tileMatrixSet=PM&url={}&http-header:referer=".format(
                self.layer_srs,
                self.layer_format,
                self.layer_name,
                self.layer_style_name,
